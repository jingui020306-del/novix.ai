...
class ContextEngine:
    ...
    def build_manifest(self, project_id: str, chapter_id: str, scene: dict[str, Any], constraints: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        examples = self.kb.query(project_id, "kb_style", query_text, top_k=int(policy.get("max_examples", 4)), filters={"asset_ids": active_assets} if active_assets else {})
        for ex in examples:
        ...
        manifest = {
            ...
            "included_evidence_chunks": {"style_examples": examples, "draft_summaries": chapter_meta.get("scene_summaries", [])},
            "dropped_items": [],
            "compression_steps": [],
        }
        ...
        if used > budget:
            manifest["compression_steps"].append("replace_history_with_summaries")
            manifest["included_evidence_chunks"]["draft_summaries"] = [{"chapter_summary": chapter_meta.get("chapter_summary", "")[:200]}]

        examples_tokens = approx_tokens(str(examples))
