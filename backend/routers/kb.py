...
@router.post('/query_multi')
def query_multi(project_id: str, body: dict, kb: KBService = Depends(get_kb)):
    return kb.query_multi(project_id, body.get('query', ''), int(body.get('top_k', 12)), body.get('kb', []), body.get('filters'))
